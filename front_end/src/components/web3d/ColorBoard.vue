    ...
    <div id="blocker">
      <div id="instructions">
        ...
        <p>
          ...
          Jump: SPACE<br/>
          Look: MOUSE<br/>
          Welcome to colorBoard! Here you can alter the color of the board as you wish. <br/>
          By pressing "E", you can increase the value of corresponding color component by one. <br/>
          By pressing "Q", you can decrease the value of corresponding color component by one. <br/>
          Focus on the board and press "E", the exact color will be shown in your chatBoard <br/>
          Enjoy yourself!
        </p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    initSocket() {
      ...
      this.socket.on("alterColor", ({red, green, blue}) => {
        colorBoard.material.color.setHex(red * 65536 + green * 256 + blue)
      })
      ...
      this.socket.emit("joinRoom", {username: this.$store.state.username, room: "colorBoard", model: this.model})
    },
    initScene() {
    ...
    initCharacter() {
      ...
      loader.load(this.model, function (mesh) {
        ...
        _this.character.rotation.y = -Math.PI / 2
    ...
    initObjects() {
      ...
      const wall0 = new THREE.Mesh(new THREE.PlaneGeometry(100, 50), wallMaterial)
      wall0.name = "wall0"
      wall0.rotation.set(0, 0, 0)
      wall0.position.set(50, 25, 0)

      const wall1 = new THREE.Mesh(new THREE.PlaneGeometry(100, 50), wallMaterial)
      wall1.name = "wall1"
      wall1.rotation.set(0, Math.PI, 0)
      wall1.position.set(50, 25, 50)

      const wall3 = new THREE.Mesh(new THREE.PlaneGeometry(50, 50), wallMaterial)
      wall3.name = "wall3"
      wall3.rotation.set(0, Math.PI / 2 * 3, 0)
      wall3.position.set(100, 25, 25)

      const floor = new THREE.Mesh(new THREE.PlaneGeometry(100, 50), floorMaterial)
      floor.name = "floor"
      floor.rotation.set(Math.PI / 2, 0, 0)
      floor.position.set(50, 0, 25)

      // 红色分量按钮
      this.loadObject(serverUrl + "models/button.obj", "redButton", function () {
        const button = scene.getObjectByName("redButton")
        button.position.set(30, 2, 10)
        button.children[0].material.color.setHex(0xff0000)
        _this.objects.push(button.children[0])
        _this.interactions.push(button.children[0])
        button.children[0].up = function () {
          _this.socket.emit("rUp")
        }
        button.children[0].down = function () {
          _this.socket.emit("rDown")
        }
      })

      // 绿色分量按钮
      this.loadObject(serverUrl + "models/button.obj", "greenButton", function () {
        const button = scene.getObjectByName("greenButton")
        button.position.set(30, 2, 25)
        button.children[0].material.color.setHex(0x00ff00)
        _this.objects.push(button.children[0])
        _this.interactions.push(button.children[0])
        button.children[0].up = function () {
          _this.socket.emit("gUp")
        }
        button.children[0].down = function () {
          _this.socket.emit("gDown")
        }
      })

      // 蓝色分量按钮
      this.loadObject(serverUrl + "models/button.obj", "blueButton", function () {
        const button = scene.getObjectByName("blueButton")
        button.position.set(30, 2, 40)
        button.children[0].material.color.setHex(0x0000ff)
        _this.objects.push(button.children[0])
        _this.interactions.push(button.children[0])
        button.children[0].up = function () {
          _this.socket.emit("bUp")
        }
        button.children[0].down = function () {
          _this.socket.emit("bDown")
        }
      })
  ...
  mounted() {
    const _this = this
    colorBoard = new THREE.Mesh(new THREE.PlaneGeometry(50, 50), new THREE.MeshBasicMaterial({
      color: 0x000000,
    }))
    colorBoard.name = "colorBoard"
    colorBoard.rotation.set(0, Math.PI / 2, 0)
    colorBoard.position.set(0, 25, 25)
    colorBoard.up = function () {
      _this.$refs.chatboard.generateMessage("Bot", `Board's current color is 0x${colorBoard.material.color.getHexString()}`)
    }
